# coding: utf-8
#程式目的:輸出高三增能A班資料集根據single variable中每個特徵的準確度進行特徵選取的結果，並輸出資料集
...
for filename in filename_list:
    ...
    auc_dataset = pd.read_csv('chka_result/classifier_' + filename + '.csv', sep=',')
    ...
    for i in range(0,len(algorithm_header)):
        ...
        metrics.to_csv('chka_result/' + result_folder[i] + '/ncu_data_week_' + filename + '.csv', index=False)
