...
for week in week_test:
    ...
    metrics_dataframe.to_csv('all_result_b/ridge/PCR_metrics_list_' + week + '_normalize.csv', index=False)

    metrics_dataframe = pd.DataFrame(each_data_list, columns=['evaluation_num', 'kfold_split_num', 'label_val_predict',
                                                              'label_val_test'])
    metrics_dataframe.to_csv('all_result_b/ridge/residual/cross_validation_Residual_table_' + week + '_normalize.csv', index=False)

    datasets = pd.read_csv('all_result_b/ridge/PCR_metrics_list_' + week + '_normalize.csv', sep=',')
    ...
    # 計算各項指標
    datasets = pd.read_csv('all_result_b/ridge/residual/cross_validation_Residual_table_' + week + '_normalize.csv',
                           sep=',')
    ...
    metrics_dataframe.to_csv('all_result_b/ridge/performance_' + week + '.csv', index=False)