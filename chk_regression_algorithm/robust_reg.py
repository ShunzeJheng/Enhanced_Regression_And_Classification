...
#進行模型訓練與評估
week_num = ['1-1', '1-2', '1-3', '1-4', '1-5', '1-6', '2-3', '2-4', '2-5', '2-6', '3-4', '3-5', '3-6', '4-5', '4-6', '5-6']
week_test = ['1-6']
for week in week_test:
    # 讀取資料並指定特徵與LABEL
    datasets = pd.read_csv('all_data_b/ncu_data_week_' + week + '.csv', sep=',')
    ...
    metrics_dataframe.to_csv('all_result_b/RANSAC_reg/PCR_metrics_list_' + week + '_normalize.csv', index=False)

    metrics_dataframe = pd.DataFrame(each_data_list, columns=['evaluation_num', 'kfold_split_num', 'label_val_predict',
                                                              'label_val_test'])
    metrics_dataframe.to_csv('all_result_b/RANSAC_reg/residual/cross_validation_Residual_table_' + week + '_normalize.csv',
                             index=False)

    datasets = pd.read_csv('all_result_b/RANSAC_reg/PCR_metrics_list_' + week + '_normalize.csv', sep=',')
    ...
    for item in values:
        result.append(
            [item[0], item[1], item[2] / 10, item[3] / (data_num - total_features), item[4] / 10, item[5] / 10])
    ...
    # 計算各項指標
    datasets = pd.read_csv('all_result_b/RANSAC_reg/residual/cross_validation_Residual_table_' + week + '_normalize.csv',
                           sep=',')
    ...
    metrics_dataframe.to_csv('all_result_b/RANSAC_reg/performance_' + week + '.csv', index=False)