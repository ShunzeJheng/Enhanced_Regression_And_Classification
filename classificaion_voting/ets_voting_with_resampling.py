# coding: utf-8
#程式目的:地科班資料集先使用resampling平衡資料集，然後使用7種分類演算法建立模型並加入投票機制
...
def read_file(algo):
    dataset = pd.read_csv('ets_data/' + algo + '/ncu_data_week_1-18(3a).csv', sep=',')
...
def oversampling(index, X, Y):
    if index == 0:
        ...
        ada = ADASYN(random_state=0,ratio='minority')
        try:
        ...
        nm1 = NearMiss(random_state=0,ratio='not minority',n_neighbors=2)
        try:
        ...
        enn = EditedNearestNeighbours(random_state=0,ratio='not minority',n_neighbors=2)
        try:
        ...
        renn = RepeatedEditedNearestNeighbours(random_state=0,ratio='not minority',n_neighbors=2)
        try:
        ...
        allknn = AllKNN(random_state=0,ratio='not minority',n_neighbors=2)
        try:
...
oversampler_name_list = ['rus', 'nm1', 'enn', 'renn', 'allknn']
for oversampler_index in range(0,len(oversampler_name_list)):
    ...
    for evaluation_num in range(1, number_of_cv_evaluation + 1):
        ...
        #metrics_dataframe.to_csv('ets_result/test.csv', index=False)
    ...
    metrics_dataframe.to_csv('ets_result/voting_result_' + oversampler_name_list[oversampler_index] + '.csv', index=False)
