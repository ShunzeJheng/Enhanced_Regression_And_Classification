...
for week in week_num:
	...
	for number_of_cv_evaluation in range(1, 301):
		...
		for evaluation_num in range(number_of_cv_evaluation):
			...
			for train_index, test_index in kfold.split(features_val):
				...
				#metrics_list.append([evaluation_num, kfold_split_num, MSE, R_squared])
	...
	final_metrics_dataframe.to_csv('all_result_b/number_of_cv_evaluation_' + week + '_normalize.csv', index=False)
	...
	plt.savefig('all_result_b/number_of_cv_evaluation_' + week + '_normalize.png')
	plt.clf()