...
def cross_validation(file, features_begin_index, features_end_index):
    ...
    for filename in filename_list:
        # 讀取資料並指定特徵與LABEL
        datasets = pd.read_csv('C:/Users/kslab/PycharmProjects/ets_PCR/data/ncu_data_week_' + filename + '.csv', sep=',')
        ...
        for evaluation_num in range(1, number_of_cv_evaluation + 1):
            ...
            for train_index, test_index in kfold.split(features_val):
                ...
                for number_of_comp in range(1, total_features + 1):
                    ...
                    MAPC = 1 - np.mean(abs((label_val_predict - label_val_test) / np.mean(label_val_test)))
        ...
        metrics_dataframe.to_csv('C:/Users/kslab/PycharmProjects/ets_PCR/cross_validation/result/PCR_metrics_list_' + filename +'_normalize.csv', index=False)

        # 計算各項指標
        datasets = pd.read_csv('C:/Users/kslab/PycharmProjects/ets_PCR/cross_validation/result/PCR_metrics_list_' + filename +'_normalize.csv', sep=',')
