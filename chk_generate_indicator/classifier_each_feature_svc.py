# coding: utf-8
#程式目的:輸出高三增能資料集在不同參數下的SVC模型對indicator的準確度
    ...
    for kernel_type in kernel_types:
        ...
        for features_begin_index in range(1,features_num+1):
            for features_mid_index in range(features_begin_index,features_num+1):
                ...
                clf = SVC(random_state=0,kernel=kernel_type).fit(features_val_standard, goal_cluster)
        ...
        if del_flag:
            metrics_dataframe.to_csv('classifier_result_a/svc_result/svc_result_' + kernel_type + '_' + filename + '_delete.csv', index=False)
        else:
            metrics_dataframe.to_csv('classifier_result_a/svc_result/svc_result_' + kernel_type + '_' + filename + '.csv', index=False)