# coding: utf-8
#程式目的:輸出高三增能資料集在不同參數下的knn模型對indicator的準確度
    ...
    for k in range(2,12):
        ...
        for features_begin_index in range(1,features_num+1):
            for features_mid_index in range(features_begin_index,features_num+1):
                ...
                clf = KNeighborsClassifier(n_neighbors=k).fit(features_val_standard, goal_cluster)
        ...
        if del_flag:
            metrics_dataframe.to_csv('classifier_result_a/knn_result/knn_result_' + str(k) + '_' + filename + '_delete.csv',
                                     index=False)
        else:
            metrics_dataframe.to_csv('classifier_result_a/knn_result/knn_result_' + str(k) + '_' + filename + '.csv',
                                     index=False)