# coding: utf-8
#程式目的:輸出高三增能資料集在不同參數下的RF模型對indicator的準確度
...
for filename in filename_list:
    ...
    for criterion_type in criterion_types:
        ...
        if del_flag:
            metrics_dataframe.to_csv('classifier_result_a/rf_result/rf_result_' + criterion_type + '_' + filename + '_delete.csv', index=False)
        else:
            metrics_dataframe.to_csv('classifier_result_a/rf_result/rf_result_' + criterion_type + '_' + filename + '.csv', index=False)