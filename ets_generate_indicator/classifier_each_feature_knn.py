# coding: utf-8
#程式目的:輸出地科班資料集在不同參數下的knn模型對indicator的準確度
...
for filename in filename_list:
    ...
    for k in range(2,12):
        ...
        if del_flag:
            metrics_dataframe.to_csv('classifier_result/knn_result/knn_result_' + str(k) + '_' + filename + '_delete.csv',
                                     index=False)
        else:
            metrics_dataframe.to_csv('classifier_result/knn_result/knn_result_' + str(k) + '_' + filename + '.csv',
                                     index=False)