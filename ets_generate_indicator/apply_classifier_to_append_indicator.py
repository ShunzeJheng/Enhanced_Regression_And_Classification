# coding: utf-8
#程式目的:輸出地科班資料集在不同模型下對indicator的預測結果，並將結果append到資料集上
...
filename_list = ["1-6(1a)"]
filename_list = ["1-12(2a)"]
filename_list = ["7-12(2d)"]
filename_list = ["13-18(3d)"]
...
for filename in filename_list:
    ...
    features_end_index = 4
    features_mid_index = 4
...
##################################################################################################
    ...
    #clf = KNeighborsClassifier(n_neighbors=4).fit(features_val_standard, goal_cluster)
    #clf = LinearSVC(random_state=0, loss='hinge').fit(features_val_standard, goal_cluster)
    clf = SVC(random_state=0, kernel='rbf').fit(features_val_standard, goal_cluster)
    ...
    #joblib.dump(clf, 'result/svc.pkl')
    ...
    metrics_dataframe.to_csv('result/ncu_data_week_' + filename + '.csv', index=False)
